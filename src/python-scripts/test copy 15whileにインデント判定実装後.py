...
def p_assign_array(p):
    ...
    if scope == 'local':                # 代入される側が local にある
        localvars[arrname][number] = value
    elif scope == 'global':             # 代入される側が global にある
        globalVars[arrname][number] = value
    else:                               # None (未定義の変数に代入しようとした場合はエラー)
...
def p_while(p):  # while (low <= high)
    ...
    if new == True:
        ...
        # endwhileの行を取得
        endline = None
        depth = 0                               # ネストの深さ (endwhileを探しているときにwhileを見つけたら元のwhileのendwhileまで無視するため)
        i = currentLine + 1                     # 次の行から
        n = len(codes)        
        # while文のネストに入ったら、endwhileでネストを抜けるまでスルーさせる
        while i < n:                            # 全ての行を越えない  (保険)
            code = codes[i].lstrip()            # 次に見る行から空白を取り除く
        ...
        if endline is None:                     # 最後までendwhileが見つからなかった
            error_msg = f"対応するendwhileが見つかりません。\nインデントなどを確認してください。"
        ...
        # while i < n:            # 全ての行を越えない  (保険)
        #     code = codes[i].lstrip()            # 次に見る行から空白を取り除く
        #     if code.startswith('while'):        # 別のwhile文 (ネストに入った) 
        #         depth += 1                      # depthが1以上ならスキップ中なのでendwhileまでスルー
        #     elif code.startswith('endwhile'):   # endwhile
        #         if depth == 0:                  # ネスト内でない
        #             endline = i                 
        #             break
        #         depth -= 1                      # ネスト内だったら抜ける
    ...
    if condition != True:                    # 条件を満たさなかった(whileの終了)
        p.parser.while_skip = True          # endwhileまで飛ばす
    # 満たしていたら、何もしない
...
def p_value_variable(p):
    ...
    if scope == 'local':            # localに有る
        p[0] = localvars[name]      # valueに値を入れて渡す
    elif scope == 'global':         # globalに有る
        p[0] = globalVars[name]     # valueに値を入れて渡す
...
def p_value_array_val(p):
    ...
    # 配列を取得
    if scope == 'local':            # localに有る
        array = localvars[name]
    elif scope == 'global':         # globalに有る
        array = globalVars[name]
...
def p_value_array_length(p):
    ...
    # 返り値を取得
    if scope == 'local':            # localに有る
        length = len(localvars[name])
    elif scope == 'global':         # globalに有る
        length = len(globalVars[name])
...
# 新しいfor文ならparserからステートを入れる
    ...
    # 条件を満たしていたらカウンタ変数を確定
        ...
        if cntscope == "local":                         # カウンタ変数がlocal
            callStack[-1]["variables"][cntname] = cntval
        else:                                           # カウンタ変数がglobal
            globalVars[cntname] = cntval
...
# currentLineがendwhileならwhileの行に戻る (endwhileの行は実行しない)
