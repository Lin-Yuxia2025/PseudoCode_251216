...
def dbg(title, **kv):
    """
    デバッグ出力: 見出し + key=value を stderr に整形して出す
    """
    lines = [f"[DBG] {title}"]
    for k, v in kv.items():
        try:
            lines.append(f"  - {k}: {pformat(v, width=120, compact=True)}")
        except Exception:
            lines.append(f"  - {k}: <unprintable>")
    print("\n".join(lines), file=sys.stderr)

def dump_top_frame(prefix=""):
    """
    callStack のトップフレーム概要を出力（無ければその旨表示）
    """
    if callStack:
        top = callStack[-1]
        dbg(prefix or "TopFrame", 
            funcName=top.get("funcName"), 
            returnLine=top.get("returnLine"), 
            endLine=top.get("endLine"),
            funcAction=top.get("funcAction"),
            localVars=list(top.get("variables", {}).keys()))
    else:
        dbg(prefix or "TopFrame", info="callStack is empty")
...
tokens = ('CIRCLE', 'TYPE', 'ARRAY', 'VALUE', 'END', 'NAME', 'NUMBER', 'COLON', 'EQUAL', 'ASSIGN', 'ADD', 'PLUS',
        ...
        'R_C_BRACKET', 'L_PAREN', 'R_PAREN', 'COMMA', 'RETURN', 'IF', 'ELSEIF', 'ELSE', 'ENDIF')
...
def p_declaration(p):
    ...
    if localvars is not None:              # 関数が呼び出されている
        ...
        dbg("DECL local", name=name, init="未定義")
    else:                                  # 関数が呼び出されてない
        globalVars[name] = "未定義"        # globalに宣言
        dbg("DECL global", name=name, init="未定義")
...
def p_multi(p):
    ...
    # ▼ デバッグ：オペランド参照直前の状態
    dbg("MULTI before",
        target=namea, left=nameb, right=namec,
        scope_target=scopea, scope_left=scopeb, scope_right=scopec,
        locals_keys=list(localvars.keys()) if localvars else None)
    ...
    # ▼ デバッグ：値と型
    dbg("MULTI values",
        left_value=valueb, right_value=valuec,
        left_type=type(valueb).__name__, right_type=type(valuec).__name__)

    #乗算して代入
    if scopea == 'local':            # localに有る
        localvars[namea] = valueb * valuec
        after = dict(localvars)
    elif scopea == 'global':         # globalに有る
        globalVars[namea] = valueb * valuec
        after = dict(globalVars)
    else:                           # 未定義の変数に乗算しようとした場合はエラー
        print(f"エラー: 変数 '{namea}' が未定義です", file=sys.stderr)

    # ▼ デバッグ：代入後スナップショット
    dbg("MULTI after", assigned_to=namea, snapshot_after=after)
...
def p_function_call(p):
    ...
    dbg("CALL (no-assign)",
        ...
        args_map=args_map,
        new_local=new_local,
        returnLine=p.parser.returnLine,
        endLine=p.parser.endLine,
        jump=p.parser.jump,
...
def p_return(p): 
    ...
    # ▼▼▼ DEBUG 追加 ▼▼▼
    dbg("RETURN (no value)", currentLine=currentLine)
...
def p_return_val_name(p):
    ...
    # ▼ デバッグ：どのスコープから値を取るか
    dbg("RETVAL name before",
        name=name,
        scope=scope,
        local_keys=list(localvars.keys()) if localvars else None,
        global_keys=list(globalVars.keys()))
    ...
    # ▼ デバッグ：返す値と型
    dbg("RETVAL name after", name=name, value=value, value_type=type(value).__name__)
...
def p_return_val(p): 
    ...
    # ▼▼▼ DEBUG 追加 ▼▼▼
    dbg("RETURN (value)", value=value, currentLine=currentLine)
...
if hasattr(parser, 'jump'):         # parserにjumpがあるか（構文解析してjump行へのスキップが指定されたか)
    ...
    dbg("JUMP set", currentLine=currentLine)
elif hasattr(parser, 'if_skip'):    # if文のスキップ処理
    currentLine = if_nextline(currentLine)
    dbg("IF skip set", currentLine=currentLine)
else:
    currentLine += 1
    dbg("LINE++", currentLine=currentLine)

# 呼び出していた関数の終了
# callStackがある　＆　次の行が呼び出した関数を越えた or returnした
# 呼び出していた関数の終了（while ではなく if で一段だけ処理）
# 条件: callStack があり、かつ（endLine を越えた or return が発生した）
if callStack and (currentLine >= callStack[-1]["endLine"] or hasattr(parser, 'ret')):
    dump_top_frame("POP before")
    endFunc = callStack.pop()  # この時点でフレームはスタックから消える

    funcAction = endFunc.get("funcAction")
    if funcAction is not None and funcAction.get("type") == "assign":
        name = funcAction["name"]
        value = getattr(parser, 'retValue', None)
        scope = funcAction["scope"]

        # ▼ デバッグ：代入前の呼び出し元ローカル辞書のスナップショット
        before_local = {}
        if scope == "local" and isinstance(funcAction["scope_ref"], dict):
            before_local = dict(funcAction["scope_ref"])  # シャローコピー
        dbg("ASSIGN before",
            assign_to=name,
            scope=scope,
            value=value,
            caller_locals_before=before_local)

        # ★ 実際の書き戻し処理
        if scope == "local":
            localVar = funcAction["scope_ref"]  # 呼び出し元のローカル辞書参照
            localVar[name] = value

            # ★ ここを追加：次リクエストに渡る callStack 側の辞書も必ず更新
            if callStack:                        # いまのトップは呼び出し元フレーム
                callStack[-1]["variables"] = localVar

        else:
            globalVars[name] = value

        # ▼ デバッグ：代入後の呼び出し元ローカル辞書
        after_local = {}
        if scope == "local" and isinstance(funcAction["scope_ref"], dict):
            after_local = dict(funcAction["scope_ref"])
        dbg("ASSIGN after",
            assign_to=name,
            scope=scope,
            caller_locals_after=after_local)

    # 呼び出し元の次の行へ復帰
    dbg("FUNC END", funcName=endFunc.get("funcName"),
        returnLine=endFunc.get("returnLine"), endLine=endFunc.get("endLine"))
    currentLine = endFunc["returnLine"]
    dbg("RESUME caller", currentLine=currentLine)

    # このフレームで使い切った return 情報をクリア
