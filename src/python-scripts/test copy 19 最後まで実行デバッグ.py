...
print("runButton in python:", runButton, file=sys.stderr)  # ★追加
...
while True:
    # ★最初で一時フラグをクリア
    for attr in [
        'jump', 'if_skip', 'while_skip',
        'dowhile_end', 'dowhile_loop',
        'newIf', 'if_Entered', 'if_End',
        'for_Entered', 'new_While',
        'doWhile', 'ret'
    ]:
    ...
    # ★★★ デバッグ: パース前の状態 ★★★
    print(
        f"[BEFORE PARSE] line={currentLine}, code={code!r}, "
        f"callStack_depth={len(callStack)}, "
        f"top_call={callStack[-1] if callStack else None}"
    )
    ...
    print(
        f"[AFTER PARSE] line={currentLine}, flags={debug_flags}"
    )
    ...
    if hasattr(parser, 'funcName'):                     # 構文解析して関数を呼び出したか(関数名があるか)
        ...
        # ★★★ デバッグ: 関数呼び出し時の情報 ★★★
        print(
            f"[CALL PUSH] currentLine={currentLine}, "
            f"funcName={parser.funcName}, "
            f"returnLine={parser.returnLine}, endLine={parser.endLine}, "
            f"args={parser.new_local}",
            file=sys.stderr
        )
        ...
        # 一度 push したら、この行以降では古い情報を使わせないように消す
        delattr(parser, 'funcName')
        if hasattr(parser, 'new_local'):
            delattr(parser, 'new_local')
        if hasattr(parser, 'returnLine'):
            delattr(parser, 'returnLine')
        if hasattr(parser, 'endLine'):
    ...
    if callStack and (currentLine >= callStack[-1]["endLine"] or hasattr(parser, 'ret')):
        print(
            "[FUNC EXIT] before pop: "
            f"currentLine={currentLine}, "
            f"top={callStack[-1]}, "
            f"ret={getattr(parser, 'ret', False)}"
        )

        endFunc = callStack.pop()
        

        # 返り値がある ＋ funcAction が設定されているときだけ代入する
        if hasattr(parser, 'retValue') and endFunc.get("funcAction"):
            ret_val = parser.retValue
            action = endFunc["funcAction"]   # {"type", "name", "scope", "scope_ref"}

            # scope に応じて代入先を切り替え
            if action["scope"] == "local":
                # local の場合は scope_ref がローカル変数辞書の参照
                action["scope_ref"][action["name"]]["value"] = ret_val
            elif action["scope"] == "global":
                # global の場合は globalVars に代入
                globalVars[action["name"]]["value"] = ret_val

        # ret / retValue フラグは使い終わったので消しておくと安心（任意）
        ...
        print(
            f"[FUNC EXIT] endFunc={endFunc}, "
            f"after pop: jump to currentLine={currentLine}, "
            f"remaining_callStack_depth={len(callStack)}"
        )


    # デバッグ用ログ
    line_repr = codes[currentLine] if currentLine < len(codes) else "<EOF>"
    print(
        f"[LOOP] runButton={runButton}, currentLine={currentLine}, "
        f"code={line_repr!r}, len(codes)={len(codes)}, "
        f"has_error={hasattr(parser, 'error')}, callStack_depth={len(callStack)}",
        file=sys.stderr
    )
