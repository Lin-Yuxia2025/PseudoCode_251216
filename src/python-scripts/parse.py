...
# トークン（単語の種類）一覧
tokens = ('CIRCLE', 'TYPE', 'ARRAY', 'VALUE', 'END', 'NAME', 'NUMBER', 'COLON', 'EQUAL', 'NOT', 'NOT2', 'LESS', 'GREATER', 'LESS_EQUAL', 'GREATER_EQUAL', 'ASSIGN', 'ADD',
...
def p_assign_array(p):
    ...
    if scope == 'local':                # 代入される側が local にある
        ...
        message = f"エラー\n{currentLine + 1}行目:   配列 '{arrname}' が未定義です"
        print(message, file=sys.stderr)
        p.parser.error = message
...
def p_for(p):    # for (i を 1 から 5 まで 1 ずつ 増やす)
    ...
    if new == True:
        ...
        if cntname_scope == 'local':       # カウンタ変数が local にある
            ...
            message = f"エラー\n{currentLine + 1}行目:   変数 '{cntname}' が未定義です"
            print(message, file=sys.stderr)
            p.parser.error = message
    ...
    if iswhile == "while":
        ...
        if new == True:
            ...
            if endline is None:                     # 最後までendwhileが見つからなかった
                error_msg = f"エラー\n{currentLine + 1}行目の while に対応する endwhile が見つかりません。\nインデントなどを確認してください。"
...
def p_do_while_start(p):  # do
    ...
    if endline is None:                     # 最後までwhile(終了)が見つからなかった
        error_msg = f"エラー\n{currentLine + 1}行目の do に対応する while が見つかりません。\nインデントなどを確認してください。"
        p.parser.error = error_msg
        return
...
def p_value_array_length(p):
    ...
    if scope == 'local':            # localに有る
        ...
        message = f"エラー\n{currentLine + 1}行目:   配列 '{name}' が未定義です"
        print(message, file=sys.stderr)
        p.parser.error = message
