...
# トークン（単語の種類）一覧
tokens = ('CIRCLE', 'TYPE', 'ARRAY', 'VALUE', 'END', 'NAME', 'NUMBER', 'COLON', 'ASSIGN', 'ADD', 'PLUS', 'OUT', 'WO', 'NO', 'NI',
        'L_S_BRACKET', 'R_S_BRACKET', 'L_C_BRACKET', 'R_C_BRACKET', 'L_PAREN', 'R_PAREN', 'COMMA', 'RETURN')
...
def p_add_end_of_array(p):
    ...
    value = variables[val_array_name][number]   # variablesから追加する配列名の要素番号で値を取り出す

    # 追加される配列名が variables に存在するか確認
    if array_name not in variables:
        print(f"エラー: 配列 '{array_name}' が未定義です", file=sys.stderr)
        return
...
def p_out(p):
    ...
    if p[1] in variables:               # NAME が variables に存在するか
        output[f'出力{outc}' ] = variables[p[1]]
        
    else:
        # 未定義の変数"を"出力しようとした場合はエラー
        print(f"エラー: 変数 '{p[1]}' が未定義です", file=sys.stderr)
...
def p_function_call(p):
    ...
    # "仮引数名": 実引数 の形でvariablesに入れる(今はvariablesに入れるが、関数が終了時にリセットしたいから、後々callStateに持たせる形にしたい)
    for k, v in args_map.items():
        variables[k] = v

    p.parser.funcName = name
    # p.parser.callArgs = args_map            # 仮引数と実引数の割り当て        (今はvariablesに入れるので必要なし)
...
# 出力
# variables, output, currentLine, functions, callStackを文字列型(json)に変換して返す
