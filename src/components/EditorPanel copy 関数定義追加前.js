...
function EditorPanel({ problemCode, setResult, variables }) {
  ...
  const parseCode = async () => {
    ...
    const res = await fetch('/api/run-python', {
      ...
      // lines(コードを分けた配列) と 現在の変数 と 次に実行する行番号を JSON文字列に変換して送る
      body: JSON.stringify({ text: lines, variables, currentLine }),   
      // ↑  req.body.text (variables)(currentLine) で送った値を取り出せる
    });
